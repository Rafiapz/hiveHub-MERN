...
function EditPost() {
  ...
  const originalContent = useSelector(
    (state: RootState) => state.posts.editPostModal.data.content
  );
  const [content, setContent] = useState<string>(originalContent);
  const dispatch = useDispatch<AppDispatch>();
  const originalUrl = useSelector(
    (state: RootState) => state.posts.editPostModal.data.media.url
  );
  const urlType = useSelector(
    (state: RootState) => state.posts.editPostModal.data.media.type
  );
  const [imageUrl, setImageUrl] = useState<string>(originalUrl);
  const [error, setError] = useState<string>("");
  const [thumbnail, setThumbnail] = useState<string | null>("");
  const postId = useSelector(
    (state: RootState) => state.posts.editPostModal.data._id
  );
